...
// console.log(cellW, cellH, colCount, gridW);
        ...
        var createCell = function ($img, info) {
            var ap='', aps = info.append.split(',');
            $.each(aps, function( index, value ) {
                 ap += aps[index] ? '<img src="'+aps[index] +'">': '';
            });

            var $cell = $img.addClass('galereya-cell-img')
                .wrap('<div class="galereya-cell" data-index="'
										+ $cells.length + '"></div>')
                .parent()
        ...
        var loadData = function (next) {
            ...
            $imgs = self.find('img').each(function (i, img) {
                item = {
                    ...
                    "hlink": img.getAttribute('data-hlink') || '#',
                    "hindex": img.getAttribute('data-index') || '0',
                    "append": img.getAttribute('data-append') || '',
                };
        ...
        var buildDOM = function () {
            ...
            var $img, title, desc, hideDesc = '', hlink, hindex=0, aps, ap='', t_s='', t_e='';
            $imgs.wrapAll('<div class="galereya-grid" />').each(function (i, img) {
                $img = $(img);
                title = data[i].title;
                desc = data[i].description;
				hlink = data[i].hlink ;
                hindex = data[i].hindex;

                aps = data[i].append.split(',');
                $.each(aps, function( index, value ) {
                     ap += aps[index] ? '<img class="img-responsive" src="'+aps[index] +'">': '';
                });
                t_s = (hindex<0)?('<a href="'+hlink+'" target="_blank">')
                                :('<a href="#" onclick=switchTab("mywork",'+hindex+')>');
                t_e = '</a>';
                hideDesc = title || desc ? '' : ' style="display: none"';
                $img.addClass('galereya-cell-img')
//grace++
                    .wrap(t_s + '<div class="galereya-cell" data-index="' + i + '"></div>' + t_e)
                    .parent()
                    .append(  ap+
                        '<div class="galereya-cell-desc"'+hideDesc+'>\
